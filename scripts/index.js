...
var scEmbeds = document.querySelectorAll('iframe')
...
function checkFocus() {
    for (let i = 0; i < vids.length; i++) {
        if(document.activeElement == scEmbeds[i]) {
            adjustVideo(i);
            window.focus();
        }
    }
...
function adjustVideo(i) {
    SC.Widget(scEmbeds[i]).getPosition(function(position) {vids[i].currentTime = position/1000;});
}

function plusDivs(n) {
    document.querySelectorAll('iframe').forEach(sound => SC.Widget(sound).pause());
...
function showDivs(n) {
    ...
    x[current].style.zIndex = 3;

    if (lastSlideIndex<slideIndex && lastSlideIndex-slideIndex==-1) {
        x[prev].style.zIndex = 2;
    }

    else if (lastSlideIndex>slideIndex && lastSlideIndex-slideIndex==1) {
        x[next].style.zIndex = 2;
    }

    else if (lastSlideIndex<slideIndex) {
        x[0].style.zIndex = 3;
        x[x.length-1].style.zIndex = 2;
    }

    x[prev].style.transform = "translateX(-100vw)";
    x[current].style.transform = "translateX(0vw)";
    x[next].style.transform = "translateX(100vw)";
    //x[slideIndex-1].style.display = "block";
}

function startStop() {
    var s = SC.Widget(scEmbeds[slideIndex-1]);
    s.isPaused(function(b) { 
        if (b) {
            s.play();
            adjustVideo(slideIndex-1);
        } else {
        s.pause();
        }
    })
...
let touchstartY = 0
let touchendY = 0
let areaOfTouch = document.getElementsByClassName("filler")[0];

function handleSwipes() {
    if (Math.abs(touchendX-touchstartX) > Math.abs(touchendY-touchstartY)) {
        if (touchendX < touchstartX) plusDivs(1)
        if (touchendX > touchstartX) plusDivs(-1)
    }
    
    else {
        if (touchendY < touchstartY) handleExplore()
        if (touchendY > touchstartY) alert('down')
    }
}

areaOfTouch.addEventListener('touchstart', e => {
...
areaOfTouch.addEventListener('touchend', e => {
...
areaOfTouch.addEventListener('wheel', e => {
    if (e.deltaY>0) handleExplore()
})
